    ...
    plugins: [
      {
        ...
        async closeBundle() {
          // Build utility-worker separately after the preload build finishes.
          // utility-worker runs as a Node.js utilityProcess (not a preload
          // script), so it must be compiled independently to avoid
          // code-splitting — Electron's sandboxed preload cannot load chunks.
          await build({
            build: {
