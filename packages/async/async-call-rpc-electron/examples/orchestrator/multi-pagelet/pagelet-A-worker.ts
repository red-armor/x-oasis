...
async function boot() {
  ...
  const sharedCh = sharedConn.getChannel();
  const daemonCh = daemonConn.getChannel();
  console.log(
    `[${SELF_ID}-worker] sharedCh connected=${sharedCh.isConnected()}, daemonCh connected=${daemonCh.isConnected()}`
  );

  sharedClient = clientHost
    .registerClient('shared-rpc', { channel: sharedCh })
    .createProxy<{
  ...
  daemonClient = clientHost
    .registerClient('daemon-rpc', { channel: daemonCh })
    .createProxy<{
      echo(msg: string): Promise<string>;
  ...
  try {
    const testResult = await sharedClient.echo('boot-test');
    console.log(`[${SELF_ID}-worker] shared echo test: ${testResult}`);
  } catch (err: any) {
    console.error(
      `[${SELF_ID}-worker] shared echo test FAILED: ${err.message}`
    );
  }

  try {
    const testResult = await daemonClient.echo('boot-test');
    console.log(`[${SELF_ID}-worker] daemon echo test: ${testResult}`);
  } catch (err: any) {
    console.error(
      `[${SELF_ID}-worker] daemon echo test FAILED: ${err.message}`
    );
  }
