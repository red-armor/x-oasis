...
import { client, pageletClient, getPageletId, PageConfig } from './App';
...
function createPageApi(pageId: string): OrchestratorAPI {
  const api = client as unknown as OrchestratorAPI;
  return {
    connect: () => api.connect(pageId),
    disconnect: () => api.disconnect(pageId),
    simulateLost: () => api.simulateLost(pageId),
    getStatus: () => api.getStatus(pageId),
    killUtility: () => api.killUtility?.(pageId),
    onStateChange: api.onStateChange.bind(api),
    onReady: api.onReady.bind(api),
    onDisconnected: api.onDisconnected.bind(api),
    onReconnecting: api.onReconnecting.bind(api),
    onReconnected: api.onReconnected.bind(api),
    onReconnectFailed: api.onReconnectFailed.bind(api),
    onClosed: api.onClosed.bind(api),
  };
}
