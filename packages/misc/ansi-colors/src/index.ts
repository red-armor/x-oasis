...
// Reason for forking: terminal color helpers do not work for child-process stdout
// in some embedders (see https://github.com/chalk/chalk/issues/381). This variant
// adds a runtime on/off switch (`setUserColor`) so the host can disable colors
// per-call without rebuilding the formatter table.
...
const createColors = (enabled = true) => ({
  ...
  strikethrough:
    enabled && useColor ? formatter('\x1b[9m', '\x1b[29m') : String,
